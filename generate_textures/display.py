...
def plot_representation_error(metamer, col_wrap: Optional[int] = None):
    ...
    ps_dummy = po.simul.PortillaSimoncelli(ps_mixture.image_shape,
                                           ps_mixture.n_scales,
    ...
    fig, axes = plt.subplots(n_rows, col_wrap, sharex=True, sharey=True,
                             figsize=figsize)
    titles = [
    ...
    ylim = [0, 0]
    child_axes = []
    for t, ax, img in zip(titles, axes.flatten(), torch.cat([metamer.image, metamer.metamer])):
    ...
    rep = ps_mixture(metamer.image, remove_stats=False) - ps_mixture(metamer.metamer, remove_stats=False)
    # because this is the error plot, it should be different ylim than
    # everyone else
    ps_dummy.plot_representation(rep, ax=axes[-1, 0], ylim=False)
    axes[-1, 0].set_title("Weighted Rep(inputs) -  Rep(mixed texture metamer)",
                          y=1.05)
